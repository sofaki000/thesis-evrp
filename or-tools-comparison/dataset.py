...
def tsp_opt(points):
    ...
    :return: Optimal solution indexes logika
    """
    ...
    return np.asarray(res[1]),all_distances
...
class TSPDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {'Points':tensor, 'Solution':solution,'Distance_matrix':distance_matrix}
    ...
    def _generate_data(self):
        ...
        if self.solve:
            for i, points in enumerate(solutions_iter):
                ...
                solution, all_distances = self.solver(points)
                solutions.append(solution)
        else:
            solutions = None

        return {'Points_List':points_list, 'Solutions':solutions,'Distance_matrix':all_distances}
